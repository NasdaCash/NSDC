...
The build script we'll be using is contained in the Nasda_Cash github repository ([contrib/gitian-build.py](https://github.com/nasdacash-project/nasdacash/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Nasda_Cash repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/Nasda_Cash-Project/Nasda_Cash/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, nasdacash-detached-sigs, and nasdacash GitHub repos.
