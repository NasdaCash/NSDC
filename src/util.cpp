...
#else
    const char* pszModule = "nasdacash";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Nasda_Cash
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Nasda_Cash
// Mac: ~/Library/Application Support/Nasda_Cash
// Unix: ~/.nasdacash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NasdaCash";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "nasdacash.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nasdacash.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nasdacash.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "nasdacashd.pid"));
