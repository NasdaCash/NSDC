...
#include "qt/nasdacash/splash.h"
#include "qt/nasdacash/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("nasdacash-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nasdacash.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nasdacash.qrc)
...
/** Class encapsulating Nasda Cash startup and shutdown.
...
/** Main Nasda_Cash application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (Nasda_CashGUI)
...
{
    window = new Nasda_CashGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &Nasda_CashGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &Nasda_CashGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(Nasda_CashGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(Nasda_CashGUI::DEFAULT_WALLET);
        ...
        // Nasda_Cash: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &Nasda_CashGUI::handlePaymentRequest);
        connect(window, &Nasda_CashGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Nasda_Cash can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nasdacash_locale);
    Q_INIT_RESOURCE(nasdacash);
    ...
    /// 6. Determine availability of data directory and parse nasdacash.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Nasda Cash"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Nasda Cash"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // nasdacash: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Nasda Cash didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
