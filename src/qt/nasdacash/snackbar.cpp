...
#include "qt/nasdacash/forms/ui_snackbar.h"
#include "qt/nasdacash/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(Nasda_CashGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &Nasda_CashGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
