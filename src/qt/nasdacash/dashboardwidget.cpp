...
#include "qt/nasdacash/dashboardwidget.h"
#include "qt/nasdacash/forms/ui_dashboardwidget.h"
#include "qt/nasdacash/sendconfirmdialog.h"
#include "qt/nasdacash/txrow.h"
...
DashboardWidget::DashboardWidget(Nasda_CashGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareNasdaCash, "square-chart-nsdc");
    setCssProperty(ui->labelNasdaCash, "text-chart-nsdc");
    ...
    setCssProperty(ui->labelAmountNsdc, "text-stake-nsdc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &Nasda_CashGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &Nasda_CashGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &Nasda_CashGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("NSDC")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NSDC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            nsdc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalNsdc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesNsdc.append(nsdc);

        int max = std::max(nsdc, nsdc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(166, 99, 212));
    ...
    set0->append(chartData->valuesNsdc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalNsdc > 0) {
        setCssProperty(ui->labelAmountNsdc, "text-stake-nsdc");
    } else {
        setCssProperty(ui->labelAmountNsdc, "text-stake-nsdc-disable");
    }
    forceUpdateStyle({ui->labelAmountNsdc});
    ui->labelAmountNsdc->setText(GUIUtil::formatBalance(chartData->totalNsdc, nDisplayUnit));
