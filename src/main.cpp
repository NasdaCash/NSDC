...
#if defined(NDEBUG)
#error "Nasda_Cash cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in unasdacash) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateNSDCSupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating NSDC supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating NSDC supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 525600 && isPoSActive) {
		nSubsidy = 10 * COIN;
	}
	else if (nHeight <= 1051201 && nHeight > 525600) {
		nSubsidy = 5 * COIN;
	}
	else if (nHeight <= 1576802 && nHeight > 1051201) {
		nSubsidy = 2.5 * COIN;
	}
	else if (nHeight <= 2102403 && nHeight > 1576802) {
		nSubsidy = 1.25 * COIN;
	}
	else if (nHeight <= 2628004 && nHeight > 2102403) {
		nSubsidy = 0.625 * COIN;
	}
	else if (nHeight <= 81407174 && nHeight > 2628004) {
...
{
    util::ThreadRename("nasdacash-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> nasdacashInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                nasdacashInputs.push_back(stakeIn);
        }
        const bool hasNSDCInputs = !nasdacashInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasNSDCInputs) {
                    ...
                    for (const CTxIn& nasdacashIn : nasdacashInputs)
                        if(nasdacashIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasNSDCInputs) {
                            for (const CTxIn& stakeIn : nasdacashInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Nasda_Cash: We use certain sporks during IBD, so check to see if they are
