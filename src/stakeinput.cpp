...
bool CNasdaCashStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CNasdaCashStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CNasdaCashStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CNasdaCashStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CNasdaCashStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CNasdaCashStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CNasdaCashStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CNasdaCashStake::GetUniqueness() const
{
    //The unique identifier for a NSDC stake is the outpoint
...
CBlockIndex* CNasdaCashStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CNasdaCashStake::ContextCheck(int nHeight, uint32_t nTime)
{
